...
$result=DB::select('select count(d.iddocumento) as CantidadDocumentos, count(a.idactivdiad) as CantidadActividades,f.idestado, es.descripcion
	from  copiaflujo f, etapa e, proceso p, estado, estado es, gestion g, actividad a, documento d, detalle_proceso_departamento dp, departamento dep
		where f.idproceso = p.idproceso and g.idactivdiad = a.idactivdiad and g.iddocumento=d.iddocumento
			and dp.idproceso=:par1 and dp.iddepartamento=:par2 and f.idestado=es.idestado
			group by f.idestado,es.descripcion
  ',['par1'=>Session::get('par1'),'par2'=>Session::get('par2')]);
?>
  ...
  <div id="right-panel" class="right-panel">
    ...
    <div  id="contenido" name="contenido" class="content mt-3">
      <table class="table table-striped table-bordered" border = "5">
              <tr>
								<th>CantidadDocumentos</th>
								<th>CantidadActividades</th>
                     <th>Estado</th>
                     <th>Descripcion</th>
									            </tr>
                 @foreach($result as $u)
                 <tr>
									 <td>{{ $u->CantidadDocumentos }}</td>
                    <td>{{ $u->CantidadActividades }}</td>
                    <td>{{ $u->idestado }}</td>
										<td>{{ $u->descripcion }}</td>
